...
export default function DepositRequests() {
  ...
  const fetchDepositRequests = async () => {
    try {
      ...
      if (err.response?.status === 401) {
        ...
        setError(
          err.response?.data?.message || "Failed to fetch deposit requests"
        );
        toast.error(
          err.response?.data?.message || "Failed to fetch deposit requests"
        );
  ...
  const handleStatusUpdate = async (
    ...
    try {
      ...
      setError(
        err.response?.data?.message || `Failed to ${status} deposit request`
      );
      toast.error(
        err.response?.data?.message || `Failed to ${status} deposit request`
      );
                      ...
                      {requests.map((request, index) => {
                        return (
                          <tr
                            ...
                            } block sm:table-row mb-4 sm:mb-0 p-4 sm:p-0 rounded-lg shadow sm:shadow-none w-full max-w-xl mx-auto`}
                          >
                            <td
                              data-label="Date:"
